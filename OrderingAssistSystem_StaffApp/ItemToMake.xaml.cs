		...
		try
		{
			...
			if (response.IsSuccessStatusCode)
			{
				...
				if (orderDetails != null)
				{
					...
					list.RemoveAll(firstPendingItem =>
	firstPendingItem.MenuItem.ItemName != startedItem.MenuItem.ItemName ||
	firstPendingItem.Description != startedItem.Description ||
	startedItem.LatestTime == null ||
	!((firstPendingItem.Order.OrderDate >= startedItem.EarliestTime) &&
	  (firstPendingItem.Order.OrderDate <= startedItem.LatestTime)) &&
